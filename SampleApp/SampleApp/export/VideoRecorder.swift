...
class VideoRecorder {
  ...
  func writeFrame(time: TimeInterval) {
    ...
    var cachedTexture: CVOpenGLESTexture?
    ...
    let err = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault,
                                                           ...
                                                           GL_RGBA,
                                                           GLsizei(bufferSize.width),
                                                           GLsizei(bufferSize.height),
                                                           GLenum(GL_BGRA),
    ...
    guard err == noErr else {
      print("Failed to create CVOpenGLESTexture while writing video")
      return
    }
    
    CVPixelBufferLockBaseAddress(pixelBuffer, [])
    
    glReadPixels(0, 0, GLsizei(bufferSize.width), GLsizei(bufferSize.height), GLenum(GL_RGBA), GLenum(GL_UNSIGNED_BYTE), CVPixelBufferGetBaseAddress(pixelBuffer))
